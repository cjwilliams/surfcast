...
static void menu_load ( Window *window ) {	
	for (int i = 0; i < NUM_CONDITIONS; i++) {
		gbitmap_init_with_resource( &condition_icons[ i ], CONDITION_ICONS[ i ] );
	}
...
static void menu_unload( Window *window ) {
  menu_layer_destroy( menu_layer );
    ...
    gbitmap_deinit( &condition_icons[ i ] );
  }
}
...
static void splash_load( Window *window ) {	
  ...
  gbitmap_init_with_resource( &logo_bitmap, RESOURCE_ID_SPLASH_ICON );
